const CONFIG = {
  // Prefer the LASTFM_API_KEY Netlify environment variable. Never commit a
  // real key here; this fallback is only for a private pasted deployment.
  apiKey: '',
...
type Resource = keyof typeof CONFIG.cacheSeconds
const RESOURCES = Object.keys(CONFIG.cacheSeconds) as Resource[]
...
function jsonResponse(
  ...
  headers.set(
    ...
    cacheSeconds
      ? `public, durable, max-age=${cacheSeconds}, stale-while-revalidate=86400`
      : 'no-store'
  )
...
function getResource(url: URL): Resource {
  const queryResource = url.searchParams.get('resource')
  if (queryResource && RESOURCES.includes(queryResource as Resource)) {
    return queryResource as Resource
  }

  const finalSegment = url.pathname.replace(/\/+$/, '').split('/').at(-1) ?? ''
  if (RESOURCES.includes(finalSegment as Resource)) {
    return finalSegment as Resource
  }
...
function getPeriod(value: string | null, fallback: string) {
  if (!value) return fallback
...
function getImage(images: Array<{ size?: string; '#text'?: string }> = []) {
...
async function getRecentTracks(apiKey: string, limit: number) {
...
async function getTopTracks(apiKey: string, period: string, limit: number) {
...
async function getTopArtists(apiKey: string, period: string, limit: number) {
...
async function getData(resource: Resource, url: URL, apiKey: string) {
  switch (resource) {
    case 'user':
...
export default async function lastFm(request: Request) {
  ...
  try {
    ...
    const apiKey = Netlify.env.get('LASTFM_API_KEY') || CONFIG.apiKey
    ...
    return jsonResponse(data, 200, corsHeaders, CONFIG.cacheSeconds[resource])
  } catch (error) {
    const status = error instanceof HttpError ? error.status : 500
    const message =
      error instanceof HttpError ? error.message : 'Unexpected server error'
    if (status >= 500) console.error('Last.fm Function failed', error)
    return jsonResponse({ error: true, message }, status, corsHeaders)
  }
}

export const config = {
  path: ['/v1/music', '/v1/music/*']
}
