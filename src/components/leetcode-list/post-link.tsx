...
const TitleIcon = ({ category }: { category: string }) => {
  // No icons for leetcode posts
  return <></>
}

const DifficultyBadge = ({ difficulty }: { difficulty: string }) => {
  if (difficulty === 'easy') return <EasyBadge />
  if (difficulty === 'medium') return <MediumBadge />
  if (difficulty === 'hard') return <HardBadge />
  return null
}
      ...
      >
        <section>
          <div className="flex items-center justify-between gap-2 md:justify-start">
            ...
            <DifficultyBadge difficulty={post.type} />
            {post.test && <TestBadge />}
